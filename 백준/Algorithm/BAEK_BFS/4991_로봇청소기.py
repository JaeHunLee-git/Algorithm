...
# def bfs(i, j, dust, move):
#     q = deque()
#     tmp = []
#     q.append([i, j, dust, move, tmp])
#     while q:
#         tx, ty, dust_tmp, move_tmp, tmp_tmp = q.popleft()
#         if dust_tmp == cnt:
#             print(move_tmp)
#             return
#         for k in range(4):
...
#                 if arr[nx][ny] == "*" and [nx, ny] not in tmp_tmp:
#                     tmp_tmp.append([nx, ny])
#                     q.append([nx, ny, dust_tmp + 1, move_tmp + 1, tmp_tmp])
#                 if arr[nx][ny] == ".":
#                     q.append([nx, ny, dust_tmp, move_tmp + 1, tmp_tmp])
#     return -1
...
while True:
    ...
    if w + h:
        maps = [
            list("".join(map(str, sys.stdin.readline().rstrip()))) for _ in range(h)
        ]
        ...
        for idx, rc in enumerate(dusts):
            ...
            if not temp:  # 로봇 청소기가 방문할 수 없는 칸이 나오면 False
                ...
                is_possible = False
                break
            cleaner[idx] += temp - 1  ## 초기 visited를 1로 설정했기 때문

        if is_possible:
            dists = [
                [0] * (len(dusts)) for _ in range(len(dusts))
            ]  # 더러운 칸마다의 거리를 계산한다.
            ...
            for li in permutations(range(len(dists))):  # 순열을 이용하여 최적의 순서를 찾는다.
                ...
                for idx in range(
                    1, len(li)
                ):  # 순열을 기준으로 시작점(start), 도착점(end)을 바꿔가며 거리를 더한다.
                    end = li[idx]
