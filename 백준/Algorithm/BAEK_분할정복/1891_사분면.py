...
def sol(num_r, num_c, size, ans):
    if size == 0:
        print(ans)
        return

    if 0 <= num_r < size and size <= num_c < 2 * size:
        sol(num_r, num_c - size, size // 2, ans + "1")
    elif 0 <= num_r < size and 0 <= num_c < size:
        sol(num_r, num_c, size // 2, ans + "2")
    elif size <= num_r < 2 * size and 0 <= num_c < size:
        sol(num_r - size, num_c, size // 2, ans + "3")
    elif size <= num_r < 2 * size and size <= num_c < 2 * size:
        sol(num_r - size, num_c - size, size // 2, ans + "4")


num_r, num_c = 0, 0
find_coordinate(num, 0, num_r, num_c, (2**d) // 2)  # 좌표 찾기
num_r -= y
num_c += x
if 0 <= num_r < 2**d and 0 <= num_c < 2**d:  # 사분면 안 쪽 범위면
    sol(num_r, num_c, (2**d) // 2, "")
else:  # 범위를 벗어날 시
    print(-1)
